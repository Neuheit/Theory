...
namespace Theory.Providers.YouTube
{
    public readonly struct YouTubeTrackLoader
    {
        ...
        private static readonly Dictionary<string, IReadOnlyCollection<IYouTubeCipherOp>> YouTubeCipherOpsCache
            = new Dictionary<string, IReadOnlyCollection<IYouTubeCipherOp>>();

        private static readonly Regex PlayerConfigRegex
            = new Regex(
                "ytplayer\\.config = (?<Json>\\{[^\\{\\}]*(((?<Open>\\{)[^\\{\\}]*)+((?<Close-Open>\\})[^\\{\\}]*)+)*(?(Open)(?!))\\})"
                , RegexOptions.Compiled);

        private static readonly Regex PlayerEmbedConfigRegex
            = new Regex(
                "yt\\.setConfig\\({'PLAYER_CONFIG':(.*)}\\);"
                , RegexOptions.Compiled);

        private static readonly Regex VideoUnavalaibleRegex
            = new Regex("id=\"unavailable-message\" class=\"message\">(.*)</", RegexOptions.Singleline);

        private static readonly Regex DecipherFuncNameRegex
            = new Regex(
                "(\\w+)=function\\(\\w+\\){(\\w+)=\\2\\.split\\(\x22{2}\\);.*?return\\s+\\2\\.join\\(\x22{2}\\)}"
                , RegexOptions.Compiled);

        private static readonly Regex DecipherDefinitionNameRegex
            = new Regex("(\\w+).\\w+\\(\\w+,\\d+\\);", RegexOptions.Compiled);

        private static readonly Regex DecipherCalledFuncNameRegex
            = new Regex(@"\w+(?:.|\[)(\""?\w+(?:\"")?)\]?\(", RegexOptions.Compiled);

        private static readonly Regex SliceSwapRegex
            = new Regex("\\(\\w+,(\\d+)\\)", RegexOptions.Compiled);

        private static readonly Regex SignatureRegex
            = new Regex("/s/(.*?)(?:/|$)", RegexOptions.Compiled);

        private static readonly Regex LengthRegex
            = new Regex("clen[/=](\\d+)", RegexOptions.Compiled);

        private static readonly Regex AudioTypeRegex
            = new Regex("mime[/=]\\w*%2F([\\w\\d]*)", RegexOptions.Compiled);

        private static readonly Regex RateLimitRegex
            = new Regex("ratebypass[=/]yes", RegexOptions.Compiled);

        public static async ValueTask<Stream> LoadTrackAsync(string trackId)
        {
            var audioStreamsInfo = await GetAudioStreamsInfoAsync(trackId)
        ...
        {
            var playerconfig = await GetPlayerConfigAsync(trackId)
                .ConfigureAwait(false);
            ...
            foreach (var streamInfoDic in streamInfoDics)
            {
                    ...
                    .StartsWith("audio/", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (length <= 0)
                    {
                        length = await RestClient.GetContentLengthAsync(url)
                            .ConfigureAwait(false);
                    ...
                    var rateLimited = !RateLimitRegex.IsMatch(url);
            ...
            {
                var signatureMatch = SignatureRegex.Match(dashManifestUrl);
                var signature = signatureMatch.Groups[1]
                    .Value;
                ...
                foreach (var streamInfoXml in streamInfosXmls)
                {
                    ...
                    {
                        var id = int.Parse(streamInfoXml.Attribute("id")
                            .Value);
                        var url = streamInfoXml.Element("BaseURL")
                            .Value;

                        var lengthMatch = LengthRegex.Match(url);
                        var length = long.Parse(lengthMatch.Groups[1]
                            .Value);

                        var bitrate = long.Parse(streamInfoXml.Attribute("bandwidth")
                            .Value);

                        var typeMatch = AudioTypeRegex.Match(url);
                        var typeString = WebUtility.UrlDecode(typeMatch.Groups[1]
                            .Value);
                        var type = GetAudioType(typeString);

                        var audioEncodingString = streamInfoXml.Attribute("codecs")
                            .Value;
        ...
        {
            var videoEmbedPage = await GetVideoEmbedPageAsync(trackId)
                .ConfigureAwait(false);
            var playerConfigMatch = PlayerEmbedConfigRegex.Match(videoEmbedPage);
            var playerConfigJson = JsonDocument.Parse(playerConfigMatch.Groups[1]
                .Value);

            var playerSourceUrl = GetPlayerSourceUrl(playerConfigJson.RootElement);
            var dic = await GetVideoInfoAsync(trackId)
                .ConfigureAwait(false);
            ...
            var page = await GetVideoWatchPageAsync(trackId)
                .ConfigureAwait(false);

            var match = PlayerConfigRegex.Match(page);

            var playerconfig = match.Groups["Json"]
                .Value;

            // if playerconfig is null or empty throw an exception
            if (string.IsNullOrWhiteSpace(playerconfig))
            {
                match = VideoUnavalaibleRegex.Match(page);
            ...
            GetCipherOpsAsync(string playerSourceUrl)
        {
            ...
            if (YouTubeCipherOpsCache.TryGetValue(playerSourceUrl, out var cacheCiphers))
                return cacheCiphers;

            var playerSource = await RestClient
            ...
            var decipherFuncNameMatch = DecipherFuncNameRegex.Match(playerSource);
            var decipherFuncName = decipherFuncNameMatch.Groups[1]
                .Value;
            ...
            var decipherFuncBody = decipherFuncBodyMatch.Groups[1]
                .Value;
            ...
            var decipherDefinitionNameMatch = DecipherDefinitionNameRegex.Match(decipherFuncBody);
            var decipherDefinitionName = decipherDefinitionNameMatch.Groups[1]
                .Value;
            ...
            var decipherDefinitionBody = decipherDefinitionBodyMatch.Groups[0]
                .Value;
            ...
            {
                var calledFuncNameMatch = DecipherCalledFuncNameRegex.Match(statement);
                var calledFuncName = calledFuncNameMatch.Groups[1]
                    .Value;
                ...
                if (isSlice)
                {
                    var indexMatch = SliceSwapRegex.Match(statement);
                    var index = int.Parse(indexMatch.Groups[1]
                        .Value);
            ...
            YouTubeCipherOpsCache[playerSourceUrl] = deciphersOps;
            return deciphersOps;
        }

        private static async ValueTask<string> GetVideoWatchPageAsync(string trackId)
        {
            var page = await RestClient
                .WithUrl(BASE_URL)
                .WithPath("watch")
        ...
        {
            var paramsEncoded = input.TrimStart('?')
                .Split("&");
